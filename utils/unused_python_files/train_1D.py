...
def train_step(DOE_phase_vector, optics_optimizer, scheduler, total_steps, psf_far, far_depth, near_depth, epoch, args):
    param = args.param
    DOE_phase = create_2d_circular_phase_map(DOE_phase_vector, args.param.R)
    ...
    # Increment target_radius based on the epoch number
    target_radius_increment = epoch // 50  # Increment radius every 50 epochs
    target_radius = min(2 + 5 * target_radius_increment, 200)  # Cap radius at 200
    ...
    # total loss
    total_loss = far_scale*far_loss + near_scale*near_loss

    # log loss, learning rate, and current target_radius
    print('\nEpoch: {}, Far Loss: {:.2f}, Near Loss: {:.2f}, Target Radius: {}'.format(epoch, far_scale*far_loss, near_scale*near_loss, target_radius))
...
def train(args):
    ...
    DOE_phase_vector = Variable(torch.zeros(args.param.R // 2, device=args.device), requires_grad=True)
    optics_optimizer = optim.Adam([DOE_phase_vector], lr=args.optics_lr)
    scheduler = StepLR(optics_optimizer, step_size=75, gamma=0.9)
    ...
    for epoch in trange(args.n_epochs, desc='Epoch'):
        step_loss = train_step(DOE_phase_vector, optics_optimizer, scheduler, total_step, psf_far, far_depth, near_depth, epoch, args)
        ...
        if total_step % args.save_freq == 0:
            DOE_phase = create_2d_circular_phase_map(DOE_phase_vector, args.param.R)
            torch.save(DOE_phase, os.path.join(args.result_path,'DOE_1D_phase_%03d.pt' % (total_step//args.save_freq)))
...
def main():
    ...
    parser.add_argument('--optics_lr', default= 10, type=float, help='optical element learning rate')
