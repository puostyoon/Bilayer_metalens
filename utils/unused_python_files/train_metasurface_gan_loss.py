...
from models.GAN_loss_functions import *
...
def backward_psf_loss(DOE_phase, far_depth, near_depth, offset_indices, args):
    ...
    # total loss
    # debugging
    total_loss = (-near_loss) * args.psf_loss_weight
    # debugging
    total_loss.backward()
        
    return total_loss.detach()

def backward_image_loss(args, batch_data, DOE_phase, wvls, thetas, net=None, discriminator=None):
    """
    ...
    for (theta_y, theta_x) in thetas:
        for wvl in wvls:
            ...
            _, _, mask, _, _, _, img_conv, psf_near, psf_far = image_formation(image_far,
                                                                                DOE_train, 
            ...
            # Adjust brightness
            scale_factor_brightness = torch.clamp(image_far.mean() / img_conv.mean(), max=args.brightness_clamp).item()
            ...
            d_real = discriminator(image_far.detach())
            d_fake = discriminator(img_conv.detach())
            # real -> D(real) >= 1 유도
            real_loss = torch.mean(F.relu(1.0 - d_real))
            # fake -> D(fake) <= -1 유도
            fake_loss = torch.mean(F.relu(1.0 + d_fake))
            d_loss = real_loss + fake_loss
            loss = loss + d_loss

            ############# Generator GAN Loss #############
            d_fake = discriminator(img_conv)
            # Generator는 D(fake)가 크게(양수) 되도록 => - mean(D(fake)) 최소화
...
def train(args):
    ...
    discriminator = PatchDiscriminator(in_channels=3, num_filters=64).to(args.device)
    ...
    for epoch in tqdm(range(args.n_epochs), position=0, leave=False):
        for step, batch_data in enumerate(tqdm(trainloader)):
            image_loss = backward_image_loss(args, batch_data, DOE_phase, wvls, thetas, net, discriminator)
            psf_loss = 0
