...
def save_settings(args, param, exist_ok=False):
    ...
    shutil.copy('./train_synthetic_aperture_metasurface.py', args.result_path)
...
def evaluate_image_loss(args, DOE_phase, net, testloader, writer, total_step, single_sensor=False, far_disparity=False, concat=False):
    ...
    for step, batch_data in enumerate(testloader):
        ...
        # Add gaussian noise
        if not args.image_noise_stddev==0:
            img_conv = img_conv + (torch.randn((img_conv.shape), device=args.device) * args.image_noise_stddev).detach()
            
        # Add poission noise 
        if not args.poisson_noise_mean==0:
            img_conv = add_poisson_noise(img_conv, args.poisson_noise_mean)
