...
def train_step_backward(
    ...
    radial_grid_2d, K_lib, eval, wvls
):
    """
    ...
    return float(loss_total.detach().item()), class_logits_2d  # return logits_2d for logging
...
def train(args):
    ...
    # ----- learnable parameters (1D radial profiles) -----
    Nr = args.radial_bins if args.radial_bins is not None else param.C

    # shared across classes: (1,1,1,Nr)  <-- 너가 원한 형태
    layer1_profile_raw = torch.randn((1, 1, 1, Nr), device=args.device, requires_grad=True)
    layer2_profile_raw = torch.randn((1, 1, 1, Nr), device=args.device, requires_grad=True)

    # orderless class logits along radius: (1,K,1,Nr)
    class_logits_profile_raw = torch.zeros((1, K_lib, 1, Nr), device=args.device, requires_grad=True)

    optimizer = optim.AdamW(
        [
            {"params": [layer1_profile_raw, layer2_profile_raw], "lr": args.optics_layer_lr},
            {"params": [class_logits_profile_raw], "lr": args.optics_class_lr},
        ],
...
def main():
    parser = argparse.ArgumentParser(
        description='PSF based Obstruction-free Metasurface training (rotational symmetry param)',
    ...
    parser.add_argument('--hard_start', default=0.7, type=float)
    parser.add_argument('--warm_frac', default=0.6, type=float)
